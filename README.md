...
GitHub Pages should no longer be treated as the live production host for the archive because it cannot enforce the password gate.
...
Current swag references include tote bag, HORIZONS notepad, LARQ bottle, foldable charger, Nord fragrances, silk eye mask, hand fan, caps, Pulsio massager, HORIZONS pen, and lanyards. The live Lanyard Colour Guide uses the final five labels and meanings: Black = Aream & Co, Brown = Crew, Blue = PC & console, Green = mobile consumer, and Oatmeal = other. Brown uses the uploaded ochre asset and Green uses the uploaded sage asset until replacement files are supplied. If a replacement image is provided, keep the same filename convention or update the path in `content.json`.
...
- Staff/contact cleanup now groups people by `International Collective / I.N.C`, `Clownfish`, `Aream & Co.`, `BeGood`, `Performers`, and `Hotel / Venue`. Clownfish roles, dietary notes, and HORIZONS Hall / Studio show-operative assignments are included without exposing dates of birth.
