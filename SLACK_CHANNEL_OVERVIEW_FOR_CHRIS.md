# Slack Channel Overview For Chris

Workspace: `International Collective`

Note: Sidebar sections are for navigation only. Channel privacy controls who can see what.

## International Collective

- `#inc-main` — Main company updates
- `#inc-core-team` — Team execution
- `#inc-red-flags-urgent` — Urgent blockers
- `#inc-finance` — Finance
- `#inc-media-production` — Production
- `#inc-marketing-web-social` — Marketing + digital
- `#inc-leads-new-deals` — Sales + deals
- `#inc-systems-tech` — Software, access + tools

## Business Divisions

- `#biz-chris-manoe` — Chris business direction
- `#biz-world-main` — All world businesses
- `#biz-singers` — Singers INC business
- `#biz-dancers` — Dancers INC business
- `#biz-artists` — Artist business
- `#biz-agency-105` — Agency 105 business
- `#biz-circle` — Circle business
- `#biz-ip` — INC IP + assets

## INC Circle

- `#circle-team-inc` — Circle team
- `#circle-outreach-inc` — Circle outreach
- `#circle-content-inc` — Circle content

## HORIZONS Event

- `#horizons-main` — Main event
- `#horizons-red-flags` — Urgent event issues
- `#horizons-schedule` — Schedule
- `#horizons-production` — Production
- `#horizons-content` — Content capture
- `#horizons-podcast` — Podcast
- `#horizons-suppliers` — Suppliers
- `#horizons-entertainment` — Entertainment
- `#horizons-locations` — Locations
- `#horizons-documents` — Files + docs
- `#horizons-decisions` — Approvals

## HORIZONS Admin / Testing

- `#horizons-test` — Website alert testing

## Privacy Note

Core INC, Business Division, and INC Circle channels are private.

HORIZONS channels currently need privacy review before inviting wider users. Keep them invite-only/private unless Samuel/Chris approve a specific public exception.

