...
| Set archive env vars in the existing Vercel project and redeploy | Existing Vercel deployment is already protected by middleware, but login returns 503 until `HORIZONS_ARCHIVE_PASSWORD` and `HORIZONS_ARCHIVE_ACCESS_TOKEN` are set in Vercel | Vercel / Archive Login | Samuel / technical admin | Critical Blocker |
| Move `inc-horizons.com` DNS from GitHub Pages to Vercel | Custom domain currently returns GitHub 404 and still has GitHub Pages A/CNAME records; Vercel must provide exact DNS records after the domain is added to the project | DNS / Live Domain | Samuel / DNS admin | Critical Blocker |
...
| Barcelo Portinatx address | Accommodation is confirmed from the workbook, but the address was not in the safe imported fields | Locations / Accommodation | Samuel / Six Senses / BeGood | Address Needed |
...
| Need | Why it matters | Section affected | Owner / who can provide | Status |
| --- | --- | --- | --- | --- |
| Review remaining missing-files tracker | Remove items already done and keep active asks clean | Admin / Missing Files | Samuel / Chris | Needs Review |
| Confirm document/logo source files still needed | Keep brand/documents section clean | Documents / Brand | Samuel / Chris | Needs Review |
