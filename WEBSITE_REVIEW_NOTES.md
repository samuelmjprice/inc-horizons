...
- New swag reference images were added for tote bag, HORIZONS notepad, LARQ bottle, charger, Nord fragrances, eye mask, fan, caps, Pulsio massager, pen, and lanyards. Official lanyard references are Oatmeal, Ochre, Black, Blue, and Sage; group meanings still need confirmation.
...
- B Good is now the required company spelling across website data and docs.
...
- Latest spelling rules: use `B Good`, `Aream & Co.`, and `Clownfish` exactly.
- Latest staff/contact cleanup groups Clownfish, International Collective / I.N.C, Aream & Co., B Good, Performers, and Hotel / Venue separately. Dates of birth are excluded from team-facing website data and reports.
...
## Master Website Data Review Workbook - 30 May 2026

Created `data/HORIZONS_Master_Website_Data_Review.xlsx` as the master line-by-line review workbook for Samuel and Chris Manoe.

Purpose:

- Review the current website data from `content.json` and supporting records.
- Avoid duplicate questions by using canonical rows and cross-used section references.
- Give Samuel/Chris green correction columns, status dropdowns, priority dropdowns, voice note references, and document/file link fields.
- Provide section-specific tabs for schedule/call sheet, locations, contacts, suppliers, podcast, speakers, rehearsals, guest materials, signage, documents, Slack/comments, Cvent, admin/data health, human confirmations, and completed updates.

Important: this workbook is for review only. It does not change the live website until approved rows are returned to Codex for a later update pass.

## Final Master Workbook Import - 31 May 2026

Imported approved/final rows from:
...
- 827 master review rows read.
- 607 approved/final rows applied to `content.json`.
- 220 rows skipped because they remained Needs Confirmation, Missing Data, Waiting on Person, Conflict, Draft, TBC, Unclear, or Answered without approval.
- 14 Not Needed rows were preserved in source/admin data and marked hidden/archived for live display.
- Production Slack routes were not enabled.
- Import reports were created: `FINAL_MASTER_IMPORT_REPORT.md` and `FINAL_MASTER_IMPORT_SKIPPED_ROWS.md`.
