# Swag Section Mapping Audit

Created: 2 June 2026

| Item | Current section | Correct section | Action taken | Needs confirmation |
| --- | --- | --- | --- | --- |
| HORIZONS House check-in caps | Swag / HORIZONS House | HORIZONS House / Reception Display and Swag / Guest Materials | Updated with 200 cap quantity split and display references | No |
| HORIZONS House lanyards | Swag | Swag / Guest Materials / Check-in | Kept as lanyard reference only; group meanings removed because PDF does not confirm them | Yes, confirm if included with check-in gift and confirm lanyard colour meanings |
| Welcome hotel room drops | Room Drops | Room Drops | Updated items, date, owners, handling notes, welcome note, and references | Final photo layout still needed if not the brief page |
| Pulsio massager | Swag / Room Drops | Room Drops and display reference | Updated spelling to Pulsio and marked On Track per brief | No |
| Hand fan | Previously Room Drops leaning | HORIZONS Connect chair drop / HORIZONS Hall | Updated as hand fan chair-drop item with 170 quantity allocation | No |
| HORIZONS notepad | HORIZONS Hall / Guest Materials | HORIZONS Connect chair drop plus allocation summary | Updated quantity allocation and terminology | Leftover allocation owner |
| HORIZONS pen | HORIZONS Hall / Guest Materials | HORIZONS Connect chair drop plus allocation summary | Updated quantity allocation and terminology | Leftover allocation owner |
| HORIZONS Connect Chair Drop | Not a dedicated item | Swag & Delivery / Guest Materials / HORIZONS Hall | Added canonical item and linked allocation data | Execution team needed |
| Spare swag handling | Not clear enough | Swag & Delivery / Post-setup | Added spare handling card: collect, package neatly, send back with Michael at Jigsaw | Leftover allocation owner needed |
| Meal / menu records | Guest Materials source data | Restaurant schedules / menus | Hidden from the live Guest Materials card list, retained in source data | No |
| Namecard proof with Eve Blackwell | Not active | Guest Materials / Namecards reference only | Added as Needs Review reference and did not change contact data | Confirm historical/proof-only status |

## Mapping Rule Applied

Room Drops are hotel-room items. HORIZONS Connect chair-drop materials belong in Swag & Delivery / Guest Materials / HORIZONS Hall, not hotel Room Drops. HORIZONS House cap gifts and display cabinet references belong in HORIZONS House / Reception and Swag & Delivery. Lanyards remain references until group meanings are confirmed.
